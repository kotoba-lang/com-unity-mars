(ns unity-mars.main
  "Kotodama WASM entrypoint for the Unity_mars clean-room actor (L4) — Clojure port.
  ...
  facts (`unity_mars.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "unity_mars")
(def tier "L4")
...
(defn emit-facts
  "EAVT facts for one record: {\"unity_mars.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "unity_mars-compat" :tier tier :entities entities} 200])
...
(defn start! [] :unity_mars-compat/ready)
